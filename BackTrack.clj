(ns untitled.BackTrack)

(defn make-a-row [columns]
  (loop [count 0 row []]
    (if (= columns count)
      row
      (recur (inc count) (conj row {:north true :east true :south true :west true :visited false :searched false})))))
...
(defn carve-passages [row col j i]
  (loop [maze (assoc-in (make-a-grid row col) [j i :visited] true)
         backtrackstack [[j i]]]
